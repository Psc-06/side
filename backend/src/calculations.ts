import type { FoodItem, Dosha, FoodReport } from "./types.js";
...
export const getThermicQualityDescription = (
  ...
  const descriptions = {
    heating:
      "Heating foods increase warmth and stimulate digestion. Good for Vata and Kapha.",
    cooling:
      "Cooling foods reduce excess heat and calm inflammation. Perfect for Pitta.",
...
export const getRecommendedTime = (bestTime: string): string => {
  if (bestTime.includes("breakfast")) return "🌅 Best for breakfast";
  if (bestTime.includes("lunch")) return "☀️ Best for lunch";
  if (bestTime.includes("dinner")) return "🌙 Best for dinner";
  if (bestTime.includes("snack")) return "🥤 Good as a snack";
